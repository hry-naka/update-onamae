...
def convert_cmd(userid,password,domain,hostname,ipv4,global_ip):
    ...
    for host in hostname:
        ...
        if ipv4[i] == 'GLOBAL-IP':
            modify_cmd += f"IPV4:{global_ip}¥n."
        else:
            modify_cmd += f"IPV4:{ipv4[i]}¥n."
        i += 1
    print( f"number of host is {i}¥n")
...
def do_update(userid,password,domain,hostname,ipv4):
    ...
    login_cmd, modify_cmd = convert_cmd(userid,password,domain,hostname,ipv4,global_ip)
    print( login_cmd )
    print( modify_cmd )
    sys.exit(1)
...
if __name__ == '__main__':
    ...
    userid,password,domain,hostname,ipv4 = read_config(args.filename[0])  # filenameを読み出す
    if (int(args.interval) == 0):       
        do_update(userid,password,domain,hostname,ipv4)
    else:
        daemonize(userid,password,domain,hostname,ipv4, convert_interval(args.interval))
    # print( args.filename )
    # print( args.interval )
    # print( get_global_ip() )
